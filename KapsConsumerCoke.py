...
#c.execute("select * from public.\"WebSearches\"")
#result = c.fetchone()
...
consumer = topic.get_simple_consumer(consumer_group=b"Coke",auto_commit_enable=True)
for message in consumer:
    if message is not None:
        #print(" [key=%18s, id=%5s, offset=%2s]" % (message.value, message.partition_key, message.offset));
        sqlstmt=("INSERT INTO public.\"WebSearches\" VALUES("+ str(message.value).replace("b\"","").replace("\"","") +")")
        print(sqlstmt)
        c.execute(sqlstmt)
        db_conn.commit() 
c.close()
